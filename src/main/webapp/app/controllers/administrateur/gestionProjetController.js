...
app.controller("gestionProjetController",
    ['$scope','ngTableParams', '$filter','security', 'HTTP_METHOD','properties', 'utils','dao',
        function ($scope,NgTableParams, $filter ,security, HTTP_METHOD, properties , utils,dao ) {
            ...
            var user=$scope.user=security.checkSecurity(["Administrateur"]);
            ...
            var lister=function() {
                ...
                var task = dao.getData(properties.urlProjet, $scope.headerBasic, HTTP_METHOD.get);
            ...
            $scope.evaluateurs=[];
            var task = dao.getData(properties.urlEvaluateur, $scope.headerBasic, HTTP_METHOD.get);
            //on attent la reponse...
            task.promise.then(function (result) {
                // fin d'attente
                // erreur ?
            ...
            $scope.supprimer=function(id){
                    var task=dao.getData(properties.urlProjet+"/"+id,$scope.headerBasic,HTTP_METHOD.delete);
                    //on attent la reponse
                    task.promise.then(function(result){
                        // fin d'attente
                        // erreur ?
            ...
            $scope.ajouter=function(){
              var projet={
                ...
                var task=dao.getData(properties.urlProjet,$scope.headerBasic,HTTP_METHOD.post,projet);
                //on attent la reponse
                task.promise.then(function(result){
                    // fin d'attente
                    // erreur ?
            ...
            $scope.validerModification=function(){
                ...
                var task=dao.getData(properties.urlProjet+"/"+$scope.idProjetModifier,$scope.headerBasic,HTTP_METHOD.put,projet);
                //on attent la reponse
                task.promise.then(function(result){
                    // fin d'attente
                    // erreur ?
