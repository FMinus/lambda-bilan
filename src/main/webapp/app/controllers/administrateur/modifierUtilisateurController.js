...
app.controller("modifierUtilisateurController",
        ...
        function ($scope,$http ,security, HTTP_METHOD, properties , utils,dao ) {
            ...
            $scope.actions.modifier=function(id,role){
                var task = dao.getData(properties.urlUtilisateur + "/" + id,  $scope.headerBasic, HTTP_METHOD.get);
            ...
            $scope.validerModification=function(){
                ...
                // on lance la requete
                var task=dao.getData(url+"/"+utilisateur.idUtilisateur, $scope.headerBasic,HTTP_METHOD.put,utilisateur);
                //on attent la reponse
                task.promise.then(function(result) {
                    // fin d'attente
                    // erreur ?
