...
app.controller("listeProjetController",
    ['$scope','ngTableParams', '$filter','security', 'HTTP_METHOD','properties', 'utils','dao',
        function ($scope,NgTableParams, $filter ,security, HTTP_METHOD, properties , utils,dao ) {
            ...
            var user=$scope.user=security.checkSecurity(["Evaluateur"]);
            $scope.headerBasic=user.headerBasic;
            var idEvaluateur=user.idUtilisateur;
            ...
            //les projets de l'evaluateur

            var task = dao.getData("/evaluateurs/"+idEvaluateur+"/projets", $scope.headerBasic
                , HTTP_METHOD.get);
            ...
            //liste des collabs d'un projet
            $scope.collaborateurs=[];
            $scope.projet={};
            var listerCollab=function(projet){
                $scope.projet=projet;
                var task = dao.getData("/projets/"+projet.idProjet+"/collaborateurs", $scope.headerBasic
                    , HTTP_METHOD.get);
                //on attent la reponse...
                task.promise.then(function (result) {
                    // fin d'attente
            ...
            $scope.ajouterIntervention=function(){
                ...
                var task = dao.getData(properties.urlIntervention+"/"+$scope.projet.idProjet+"/"+$scope.collaborateur.idUtilisateur,
                    $scope.headerBasic, HTTP_METHOD.put,$scope.intervention);
                //on attent la reponse...
                task.promise.then(function (result) {
                    // fin d'attente
                    // erreur ?
            ...
            $scope.action={creerFeedback:undefined,listerCollab:listerCollab};
